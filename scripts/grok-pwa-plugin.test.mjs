...
import { HAS_GROK_PUBLIC } from "./harness-assets.mjs";
import {
  appNameFromHost,
  createHeadInjector,
...
test("platform chrome overwrites share-card metas and always sets og:title", { skip: !HAS_GROK_PUBLIC }, () => {
  const html =
...
test("published grok.me slug is still a title fallback", { skip: !HAS_GROK_PUBLIC }, () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
  });
...
test("emits og:image for a public host and prefers a custom card", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("placeholder og:image appends site.color when it is 6-digit hex", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("document title entities are not double-escaped on og:title", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("injects into documents with no head element", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("streaming injector matches </HEAD> case-insensitively", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("uses the app name in the injected title tag", { skip: !HAS_GROK_PUBLIC }, () => {
...
test("nitro middleware and its bundled assets exist", { skip: !HAS_GROK_PUBLIC }, () => {
