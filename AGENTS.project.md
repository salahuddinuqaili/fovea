...
## Current version: v12.0.0 — Work follows the write (prod exec still off, Auth still OFF)
...
- Command is desk-true: Maya’s first-run, Jordan’s queue, Riley’s audit, Alex’s grant roster. A named handoff is a banner; it does not hide that home. Labels face the recipient. After Jordan decides, Maya’s Work thread follows the write and a decision handoff lands on her desk. Personal memory is encrypted at rest in process. Team memory is scoped to the caller’s team. Sandbox UPDATE/DELETE need WHERE. Snapshot SQL is redacted.
...
- Operator simulations in `src/kernel/simulations.ts` are a release input (seventeen journeys).
