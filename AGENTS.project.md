...
## Current version: v10.0.0 — Desk-true Command / honest storage (prod exec still off, Auth still OFF)
...
- Command is desk-true: Maya’s first-run, Jordan’s queue, Riley’s audit, Alex’s grant roster. Personal memory is encrypted at rest in process. Team memory is scoped to the caller’s team. Sandbox UPDATE/DELETE need WHERE. Snapshot SQL is redacted.
...
- Operator simulations in `src/kernel/simulations.ts` are a release input (fifteen journeys).
